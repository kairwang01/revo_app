# Revo Frontend
Static multi-page storefront and trade-in UI for Revo. Everything lives in `public/` and runs as plain HTML, CSS, and JavaScript. Default API target is `https://revo-backend-o03w.onrender.com/api`.

## Quick start
- Prerequisites: Python 3 (for the simple static server), a modern browser, and access to the backend API (FastAPI).
- Configure `public/assets/js/config.js` with your backend URL and any keys (see Configuration).
...
- `FEATURES`: `USE_BACKEND` (always true for live data), `AUTO_INIT` (pings `/api/health` on load), `DEBUG_MODE` (console logging).
- `GOOGLE_CLIENT_ID` and optional `GOOGLE_AUTH_ENDPOINT` for OAuth; `ANALYTICS_ID` for GA4; `SUPPORT_API_URL` for the support launcher.
- `TIMEOUTS`: request timeouts in ms.

Frontend notes:
- Data persisted in `localStorage`: `revo_city`, `revo_city_tax`, `revo_cart`, `revo_auth`, `authToken`, `revo_user`, `revo_wallet`, `revo_orders_local`, plus address and preference keys used by checkout/settings.
...
- `public/assets/js/`: Shared modules `config.js`, `backendApi.js`, `api.js`, `storage.js`, `ui.js`, `main.js`, `geo.js` (ES module), `googleAuth.js`, `support.js`. Page controllers live in `assets/js/pages/`.
...
- Cart: Items live in `localStorage`; quantity changes and city tax are client-side.
- Checkout: Requires login. Prefers backend addresses, falls back to local storage. Calculates totals and calls `api.checkout` before `fake-payment.php`/`payment-success.html`.
...
- Trade-in: `sell.html` loads brands/models, requests an estimate, collects photos, and creates pickup requests. `admin.html` lets admins list, evaluate, and delete trade-in pickups.
...
- City + tax: `geo.js` and `storage.js` store the chosen city and tax rate, auto-detect city with geolocation, and emit `revo:city-changed` on updates. `settings.html` and `choose-city.html` allow manual selection.
- Extras: Dialogflow chat iframe on `index.html`. GA4 hooks in `support.js` (set `CONFIG.ANALYTICS_ID`). Optional support launcher hits `CONFIG.SUPPORT_API_URL`.
